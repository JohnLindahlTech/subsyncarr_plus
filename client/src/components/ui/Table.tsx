...
export const THead: React.FC<TableProps> = ({ children, className }) => (
  <thead className={clsx('bg-background-alt/50 border-b border-border', className)}>
    {children}
  </thead>
);
...
export const TH: React.FC<THProps> = ({ 
  children, className, sortable, active, order, onClick 
}) => (
  <th 
    onClick={onClick}
    className={clsx(
...
export const TD: React.FC<TDProps> = ({ children, className, ...props }) => (
  <td 
    className={clsx('px-6 py-4 text-sm font-medium text-foreground', className)}
    {...props}
  >
    {children}
  </td>
